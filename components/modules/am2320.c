...
static int _read(uint32_t id, void *buf, uint8_t len, uint8_t off)
{
    ...
    ets_delay_us(800); //os_delay_us(800);
    ...
    ets_delay_us(1500); //vTaskDelay(2 / portTICK_PERIOD_MS); //os_delay_us(1500);
    platform_i2c_send_start(id);
    platform_i2c_send_address(id, am2320_i2c_addr, PLATFORM_I2C_DIRECTION_RECEIVER, 1);    
    ets_delay_us(30); //os_delay_us(30);
    //vTaskDelay(1);
...
static int am2320_setup(lua_State* L)
{
    ...
    ets_delay_us(1500); //os_delay_us(1500); // give some time to settle things down
